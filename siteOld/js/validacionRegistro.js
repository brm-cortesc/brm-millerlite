	...
	*se efectua a traves de la libreria jquery.validate*/
		...
		jQuery("#msform").validate({
		  errorPlacement:function (error, element) {
											if (element.is("#terminos")) {
													error.appendTo(".termin #erroChkt");
											} else if (element.is("#politica")) {
													error.appendTo(".politc #erroChkp");
											} else{
		...
		jQuery('#pais').change(function(){
			...
			if(p !='46'){
				jQuery(".deptos").hide()
				jQuery("#ciudad").replaceWith('<input type="text" name="idCiudad" id="ciudad" class="form-control" placeholder="Ingresa tu Ciudad">');
			}else{
				jQuery('.deptos').show();
				jQuery("#ciudad").replaceWith('<select name="idCiudad" id="ciudad" class="form-control"> </select>');
			}
		...
		jQuery("#depto").change(function(){
			...
			jQuery.ajax({
				...
				success:function(data){
					...
					var ciudades="";
					ciudades+="<option value=''>Seleccione la ciudad</option>";
			...
			if( jQuery("#msform").valid() ){
				...
				jQuery.ajax({
					...
					success:function(data){
						jQuery(".loading").hide();
						if (data=="1") {
							dataLayer.push({'event': 'login-exitoso'});
